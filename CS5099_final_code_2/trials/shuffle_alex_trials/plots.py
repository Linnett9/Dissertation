...
def save_summary_and_report(batch_size, seed, training_time, val_loss, val_accuracy, best_val_accuracy, true_classes, predicted_classes, report_str, model, dropout_rate, learning_rate, num_dense_units, activation, optimizer, filters, classification_threshold, cond_activation_threshold, cond_activation_weight, l1_strength, l2_strength, output_dir, class_labels):
    # Save the summary
    with open(os.path.join(output_dir, f'summary_batch_size_{batch_size}_seed_{seed}_fold_{training_time}_l1_{l1_strength}_l2_{l2_strength}.txt'), 'w') as f:
        ...
        f.write(f'Number of Dense Units: {num_dense_units}\n')
        ...
        f.write(f'Conditional Activation Threshold: {cond_activation_threshold}\n')
        f.write(f'Conditional Activation Weight: {cond_activation_weight}\n')
        ...
        f.write('\nTrue Classes:\n')
        f.write(' '.join(map(str, true_classes)) + '\n')
        f.write('\nPredicted Classes:\n')
        f.write(' '.join(map(str, predicted_classes)) + '\n')
        f.write('\nClass Labels:\n')
...
def save_best_configuration(best_config, best_val_accuracy, output_dir):
    ...
    with open(os.path.join(output_dir, 'best_configuration.txt'), 'w') as f:
        f.write(f'Best Batch Size: {best_config["batch_size"]}\n')
        f.write(f'Seed: {best_config["seed"]}\n')
        f.write(f'L1 Strength: {best_config["l1_strength"]}\n')
        f.write(f'L2 Strength: {best_config["l2_strength"]}\n')
