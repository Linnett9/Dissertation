...
REMOTE_MODEL_SAVE_DIR = '/data/bl70/models/IncResMob9'
REMOTE_TUNER_RESULTS_DIR = '/data/bl70/TestingInceptionResNetMobile9'
...
def transfer_tuner_results_to_gpu(local_model_dir, remote_dir=REMOTE_TUNER_RESULTS_DIR):
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(SSH_HOST, username=SSH_USERNAME, password=SSH_PASSWORD)
    sftp = ssh.open_sftp()
    ...
    try:
        ...
        # Transfer best model
        local_model_path = os.path.join(local_model_dir, 'best_model.keras')
        ...
        local_config_path = os.path.join(local_model_dir, 'best_configuration.json')
        remote_config_path = os.path.join(remote_dir, 'best_configuration.json')
        ...
        local_results_path = os.path.join(local_model_dir, 'best_results.json')
        remote_results_path = os.path.join(remote_dir, 'best_results.json')
