...
def save_summary_and_report(batch_size, seed, fold, training_time, val_loss, val_accuracy, best_val_accuracy, true_classes, predicted_classes, report, model, l1_strength, l2_strength, output_dir):
    # Save the summary
    with open(os.path.join(output_dir, f'summary_batch_size_{batch_size}_seed_{seed}_fold_{fold + 1}_l1_{l1_strength}_l2_{l2_strength}.txt'), 'w') as f:
...
def save_best_configuration(best_config, best_val_accuracy, output_dir):
    ...
    with open(best_config_file, 'w') as f:
        ...
        for key in best_config.values.keys():
            f.write(f"{key}: {best_config.get(key)}\n")